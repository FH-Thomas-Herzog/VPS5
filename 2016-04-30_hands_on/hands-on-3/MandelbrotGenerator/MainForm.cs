...
namespace MandelbrotGenerator
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            switch (Settings.DefaultSettings.Version)
            {
                case Version.Sync:
                    generator = new SyncImageGenerator();
                    break;
                case Version.AsyncV1:
                    IAsyncImageGenerator asyncThreadGenerator = new AsyncGeneratorV1();
                    asyncThreadGenerator.OnCompleted += OnImageGenerated;
                    generator = asyncThreadGenerator;
                    break;
                case Version.AsyncV2:
                    IAsyncImageGenerator asyncWorkerGenerator = new AsyncGeneratorV2();
                    asyncWorkerGenerator.OnCompleted += OnImageGenerated;
                    generator = asyncWorkerGenerator;
                    break;
                case Version.Parallel:
                    IAsyncImageGenerator parallelWorkerGenerator = new ParallelGenerator();
                    parallelWorkerGenerator.OnCompleted += OnImageGenerated;
                    generator = parallelWorkerGenerator;
                    break;
            }
        }

        /// <summary>
        /// COmpleted observer for asynchronous image generation.
        /// </summary>
        /// <param name="sender">the generator</param>
        /// <param name="e">the event arguments</param>
        private void OnImageGenerated(object sender, EventArgs<Tuple<Area, Bitmap, TimeSpan>> e)
        {

            if (this.InvokeRequired)
            {
                this.Invoke((Action)(() => OnImageGenerated(sender, e)));
            }
            else
            {
        ...
        /// <summary>
        /// Updates the image synchronously.
        /// </summary>
        /// <param name="area">the are to generate image for</param>
        private void UpdateImage(Area area)
        {
            ...
            stopWatch.Start();
            var bitmap = generator.GenerateImage(0, area.Width, area, () => false);
            stopWatch.Stop();
            currentArea = area;
        ...
        /// <summary>
        /// Updates the image unsynchsonously
        /// </summary>
        /// <param name="area">to area to generate image for</param>
        private void UpdateImageAsync(Area area)
        {
            toolStripStatusLabel.Text = "Calculating ...";
            (generator as IAsyncImageGenerator)?.GenerateAsync(area);
        }
        ...
        private void pictureBox_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            // Calls update image depending oncurrent set version
            try
            {
                switch (Settings.DefaultSettings.Version)
                {
                    case Version.Sync:
                        ...
                        break;
                    case Version.AsyncV1:
                        (generator as IAsyncImageGenerator)?.Abort();
                        UpdateImageAsync(area);
                        break;
                    case Version.AsyncV2:
                        (generator as IAsyncImageGenerator)?.Abort();
                        UpdateImageAsync(area);
            ...
            {
                Console.WriteLine("Error on mouse up");
            }
        }
        #endregion
    }
